...
public class ImagesFragmentCopy extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        fetchImages();
        System.out.println("Second RESULT data is fetched too");
    }
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                if (jsonObject.has("mime_type")) {
                                    ...
                                    if (mime_type.compareTo("image/jpeg") == 0 || mime_type.compareTo("image/png") == 0) {
                                        ...
                                        String url = jsonObject.getString("preview");
    ...
    private void fetchImages() {
        final List<ImageRecord> imageRecords = null;
        for (int j = 0; j < records.size(); j++) {
            final int finalJ = j;
            ImageRequest imageRequest = new ImageRequest(records.get(j),
                    new Response.Listener<Bitmap>() {
                        @Override
                        public void onResponse(Bitmap response) {
                            // do something
                            ImageRecord record = new ImageRecord(records.get(finalJ));
                            imageRecords.set(finalJ, record);
                        }
